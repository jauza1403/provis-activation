...
export async function PATCH(request: Request) {
  try {
    ...
    const isReschedule = body.status === "Reschedule";
    if (user.role === "vendor_user") {
      const vendorAllowedKeys = ["id", "status", "activationDate", "timeSlot", "rescheduleReason", "pendingReason"];
      const vendorHasExtra = Object.keys(body).some((key) => !vendorAllowedKeys.includes(key));
      if (!isReschedule || vendorHasExtra || !String(body.rescheduleReason ?? "").trim()) {
        return NextResponse.json(
          { error: "Vendor hanya dapat mengubah jadwal (reschedule) beserta alasan pada request miliknya." },
          { status: 403 },
        );
      }
    }
      ...
      if (reason) {
        const prefix =
          user.role === "vendor_user"
            ? "(from vendor to provision) "
            : "(from provision to vendor) ";
        update.rescheduleReason = `${prefix}${reason}`;
      }
    }
